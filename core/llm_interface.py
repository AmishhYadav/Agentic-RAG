                ...
                return json.dumps(
                    {
                        ...
                        "reasoning": (
                            f"Query contains domain keywords "
                            f"({[k for k in keywords if k in user_prompt_lower]}) "
                            f"requiring context."
                        ),
                    }
                )
        ...
        if "synthesize an answer" in system_prompt.lower():
            if "bedrock" in user_prompt_lower:
                return (
                    ...
                    "high-performing foundation models (FMs) via a single API, along with a "
                    "broad set of capabilities you need to build generative AI applications "
                    "with security, privacy, and responsible AI."
                )
            elif "security" in user_prompt_lower or "iam" in user_prompt_lower:
                return (
                    "AWS Identity and Access Management (IAM) allows you to securely control "
            ...
            # Check if context was actually provided in the system prompt
                ...
                return (
                    "Based on the retrieved context, I can confirm that the system has found "
                    "relevant documents to answer your query regarding AWS/Bedrock/Security. "
                    "(Simulated RAG Answer)"
                )

            else:
                return (
                    "Based on the retrieved context, this is a simulated answer generated by the "
                    "MockLLM. (Context was irrelevant or missing for specific mock response)."
                )
